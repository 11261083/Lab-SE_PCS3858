...
void processBusArrivalPacketData(BusArrivalDataPacket arrivalData)
{
  ...
  if(arrivalData.stopId == 101)
  {
    BusPredictDataPacket busPredictForStop101;
    busPredictForStop101.stopId = arrivalData.stopId;
    busPredictForStop101.lineId = arrivalData.lineId;
    busPredictForStop101.busId = arrivalData.busId;
    busPredictForStop101.packetType = BUSPREDICTDATA_PACKET;
    busPredictForStop101.predictTime = arrivalData.time + 1200; // 20min * 60s = 1200s

    uint32_t unixTime = arrivalData.time + 1200;
    ...
    String hour = (String(currentHour) + ":" + String(timeStruct.tm_min) + ":" + String(timeStruct.tm_sec));
      ...
      // ENVIANDO PACOTE CHEGADA VIA MQTT P/ TAGOIO
    const size_t  capacity = JSON_OBJECT_SIZE(15);
    StaticJsonDocument<capacity> doc;
    doc["P_id_"+String(arrivalData.stopId)] = arrivalData.busId;
    doc["P_line_"+String(arrivalData.stopId)] = arrivalData.lineId;
    doc["P_date_"+String(arrivalData.stopId)] = date;
    doc["P_hour_"+String(arrivalData.stopId)] =  hour;
    doc["P_time_"+String(arrivalData.stopId)] = unixTime;
    doc["P_time_"+String(arrivalData.stopId)+ "_" + String(arrivalData.lineId)] = hour; //atencao! variavel para display
    String topic = "predicts/" + String(arrivalData.stopId);
    String mqttOutput;
    serializeJson(doc, mqttOutput);

    MQTT.publish(topic.c_str(), mqttOutput.c_str());

    BroadcastPredictData(busPredictForStop101);
  }
...
void BroadcastPredictData(BusPredictDataPacket predictData)
{
  ...
  Serial.println(predictData.predictTime);
  LoRa.endPacket();
}
